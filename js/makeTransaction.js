...
(function() {
  ...
  const txtToken = document.getElementById('tokenNumber');
  const btn2ndLogin = document.getElementById('finalLogin');
  ...
  submit.addEventListener('click', e => {
    ...
    var tokenGenerator = new FirebaseTokenGenerator("AIzaSyBpNT2BDHUnebERrbr_86rbr2zKuAwg_XE");
    token = tokenGenerator.createToken({ uid:firebaseUserID, some: "arbitrary", data: "here" },{ expires: 10 });
    token = token.substring(token.length-10, token.length);

    var principalRoute = firebase.database().ref().child('Users').child(firebaseUserID);
      var email;
      principalRoute.child('Enterprise').on('value', function(dataSnapshot) {
        if(dataSnapshot.val() == null){
          principalRoute = firebase.database().ref().child('Users').child(firebaseUserID);
        }
        else {
          principalRoute = firebase.database().ref().child('Users').child(dataSnapshot.val());
        }

        principalRoute.child('Mail').on('value', function(data){
          email = data.val();

          emailjs.send("gmail","flex_template",{to_email: email , to_name: email , message_html: "Transaction token: "+token});
        });
      });
  ...
  btn2ndLogin.addEventListener('click', e => {
    const tokenUser = txtToken.value;
    if(tokenUser != token){
      //signInSnackbar.MaterialSnackbar.showSnackbar({message: 'Bad Token', timeout: 2000});
      ...
      checkDB.once('value', snap => {
        ...
        if(destinationID != null){
            ...
            if(destinationID == firebaseUserID){
            ...
            if(!insufficient){
              ...
              checkDB.child(destinationID).child('Enterprise').on('value', function(data) {
                var r = firebase.database().ref().child('Users').child(destinationID);

                if(data.val() == null){
                 r = firebase.database().ref().child('Users').child(destinationID);
                }
                else {
                  r = firebase.database().ref().child('Users').child(data.val());

                }

                r.child('Mail').on('value', function(data2){
                  var destination = data2.val();
                 emailjs.send("gmail","flex_template",{to_email: destination , to_name: destination , message_html: "You just received a transaction from flex. This are the details: \nRecipient: "+ user+"\nRecipient Account: "+userNumber+ "\nAmount: "+amount.value+ "\n Motive/Comment: "+comment.value+ "\n Date: "+ new Date().toUTCString()+ "\n Please check your account status for official voucher." });
              });
              });
              ...
              emailjs.send("gmail","flex_template",{to_email: email , to_name: email , message_html:"We are sending you the details of your transaction: \nRecipient: "+ recipientName.value+"Recipient Account: "+recipientNumber.value+ "Amount: "+amount.value+ "Motive/Comment: "+comment.value+ "Date: "+ new Date().toUTCString()+ "In case you don’t recognize this operation, please communicate to: flextransacciones@gmail.com. Please check your account status for official voucher."});
             });
          });
  ...
  firebase.auth().onAuthStateChanged(firebaseUser => {
    if(firebaseUser){
      ...
      principalRoute.child('Enterprise').on('value', function(dataSnapshot) {
        ...
        myFavorites = principalRoute.child('FrequentAccounts');
